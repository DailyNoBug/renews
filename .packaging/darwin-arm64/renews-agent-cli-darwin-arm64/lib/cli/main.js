...
const main = async () => {
    ...
    switch (command) {
        ...
        case "index": {
            ...
            const repoMap = await new RepoMapBuilder(new TreeSitterManager()).build(path.resolve(system.config.project.workspaceRoot), system.config.context.repoMap.maxSymbols);
